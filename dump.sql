--ПРОБЛЕМЫ БД!!!!!!!!!!!!!!!!!!!!!!!!!
-- orders очень большая все в куче
-- нормализации нет
-- индексы есть, но не на все поля которые нужно
-- связи между таблицами есть, но не все
-- есть поля, которые не используются
-- даты доставки как колонки — 8 отдельных колонок
...
create index IDX_3 on orders (create_date);

create index IDX_4 on orders (create_date, status);

create index IDX_5 on orders (hash);
...
create index IDX_318C0B7C7FC358ED on orders_article (orders_id);