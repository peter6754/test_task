...
--
-- Хост: localhost:3306
-- Время создания: Апр 09 2026 г., 21:13
...
CREATE TABLE `orders` (
  ...
  `status` enum('new','confirmed','paid','in_production','shipped','delivered','cancelled') NOT NULL DEFAULT 'new' COMMENT 'Статус заказа',
...
CREATE TABLE `orders_article` (
  ...
  `multiple_pallet` enum('by_package','by_pallet','min_pallet') DEFAULT NULL COMMENT 'Кратность палете',
...
CREATE TABLE `orders_delivery_dates` (
  `id` int NOT NULL,
  `order_id` int NOT NULL,
  `type` enum('estimated','confirmed','fast_pay','old','proposed','sending','ship','fact') NOT NULL COMMENT 'Тип даты',
...
CREATE TABLE `orders_meta` (
  ...
  `offset_reason` enum('factory_holiday','factory_clarifying','other') DEFAULT NULL COMMENT 'Причина сдвига сроков',
